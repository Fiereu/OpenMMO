...
class UserAuthenticationServiceTest : ShouldSpec({
  context("UserAuthenticationService") {
    ...
    val testPasswordHash = "5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8" // SHA-1 hash of "password"
    val testPasswordBytes = byteArrayOf(
      0x5b.toByte(), 0xaa.toByte(), 0x61.toByte(), 0xe4.toByte(),
      0xc9.toByte(), 0xb9.toByte(), 0x3f.toByte(), 0x3f.toByte(),
      0x06.toByte(), 0x82.toByte(), 0x25.toByte(), 0x0b.toByte(),
      0x6c.toByte(), 0xf8.toByte(), 0x33.toByte(), 0x1b.toByte(),
      0x7e.toByte(), 0xe6.toByte(), 0x8f.toByte(), 0xd8.toByte()
    )
    val wrongPasswordBytes = byteArrayOf(0x00, 0x01, 0x02)
    val testToken = byteArrayOf(0x01, 0x02, 0x03, 0x04)
    val testTokenRecord = UserTokenRecord(
      token = testToken
    )
    val wrongTokenRecord = UserTokenRecord(
      token = byteArrayOf(0x05, 0x06, 0x07, 0x08)
    )

    context("loginPassword") {
      should("should return true when username and password match") {
        val userRecord = mockk<UserRecord> {
          every { password } returns testPasswordBytes
        }
      ...
      should("should return false when password does not match") {
        val userRecord = mockk<UserRecord> {
          every { password } returns wrongPasswordBytes
        }
        every { userRepository.getUser(testUsername) } returns Optional.of(userRecord)
